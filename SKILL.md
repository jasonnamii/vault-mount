---
name: vault-mount
description: "옵시디언 볼트(Dropbox) 자동 마운트 게이트키퍼. **모든 파일 작업·VAULT 접근·Read/Write/Edit/Bash 직전 자동선행**. 세션 첫 메시지·재개·이어서·resume·볼트·마운트·옵시디언·드롭박스·VAULT·파일·노트·메모·저장·읽기·쓰기·검색 등 트리거 hit 시 mcp__cowork__request_cowork_directory를 결정적 호출. **🆕 v1.2 PATH_TRIGGER (Rule 0·100% 강제발동·grep 우회):** 메시지에 `/Users/jason/`·`/Users/jason/Library/CloudStorage/Dropbox/ObsidianVault/`·`/Users/jason/ObsidianVault/`·`~/` (jason 컨텍스트) 등 host 절대경로 패턴이 hit 시 단어 매칭과 무관하게 즉시 발동. 3단계 폴백(코워크 빌트인→옵시디언 MCP→Desktop Commander). 마운트 실패시 1줄 보고 후 outputs 폴백으로 작업 진행(STOP ✗). UP §BOOTSTRAP과 이중 안전장치. 권한 팝업 중복 ✗ — already_mounted 검증 스킵 내장."
version: v1.2
vault_dependency: required
license: "Proprietary. LICENSE has complete terms."
P1: 볼트마운트, 볼트, vault, vault mount, vault-mount, 마운트, mount, 자동마운트, 시작, 재개, resume, 이어서, 이어하자, 볼트연결, 볼트접속, 볼트확인, 볼트체크, 볼트경로, vault path, 옵시디언, 옵시디언볼트, obsidian, obsidian vault, 옵시디언마운트, obsidian mount, 드롭박스, dropbox, 드롭박스마운트, 경로, 경로확인, FS, FS_ACCESS, 파일시스템, file system, 파일접근, 노트접근, 볼트접근, vault access, BOOTSTRAP, 부트스트랩, 게이트키퍼, gatekeeper, 자동선행, 강제선행, 마운트선행, 선행마운트, **PATH_TRIGGER (Rule 0·100% 강제):** /Users/jason/, /Users/jason/Library/CloudStorage/Dropbox/ObsidianVault, /Users/jason/Library/CloudStorage/Dropbox/ObsidianVault/, /Users/jason/ObsidianVault, /Users/jason/ObsidianVault/, ~/ (jason 컨텍스트), host절대경로, 경로패턴, path pattern, absolute path.
...
| # | 규칙 | 위반 시 |
|---|------|--------|
| 0 | **PATH_TRIGGER (100% 강제발동)** — 형 메시지에 `/Users/jason/`·`/Users/jason/Library/CloudStorage/Dropbox/ObsidianVault`·`/Users/jason/ObsidianVault`·`~/` 등 host 절대경로 패턴이 hit 시 단어 트리거와 무관하게 **즉시** 본 스킬 발동. grep 우회·NOT 우회 ✗·예외 ✗. 발동 후 ② 검증 → ③ 호출 (이미 마운트면 스킵) | 형이 경로를 박는 행위 = 100% 파일작업 신호. 미발동 = 본질 누락 |
...
**자동 발동 조건 (하나라도 hit):**
- **🆕 PATH_TRIGGER (Rule 0·최우선·100% 강제):** 메시지에 다음 경로 패턴 hit 시 즉시 발동
  - `/Users/jason/` (jason 호스트 루트)
  - `/Users/jason/Library/CloudStorage/Dropbox/ObsidianVault` (정식 경로)
  - `/Users/jason/Library/CloudStorage/Dropbox/ObsidianVault/` (정식 경로 + 슬래시)
  - `/Users/jason/ObsidianVault` (별칭 경로)
  - `/Users/jason/ObsidianVault/` (별칭 경로 + 슬래시)
  - `~/` (jason 컨텍스트의 홈 별칭)
  - 검출 정규식 (참고): `(/Users/jason/|~/)`
...
```
❌ WRONG: 형이 "/Users/jason/Library/CloudStorage/Dropbox/ObsidianVault/" 박았는데 단어 트리거 미hit → 발동 ✗
✅ CORRECT: 경로 패턴 hit = Rule 0 PATH_TRIGGER 100% 강제발동 (단어 매칭 우회·즉시 ② 검증)
```
...
**케이스 5: PATH_TRIGGER 절대경로 hit (v1.2 신규·Rule 0)**
```
사용자: "이거 처리해줘 → /Users/jason/Library/CloudStorage/Dropbox/ObsidianVault/Agent-Ops/메모.md"
hit: PATH_TRIGGER `/Users/jason/Library/CloudStorage/Dropbox/ObsidianVault/` (Rule 0·100%)
→ 단어 매칭 우회 → 즉시 ② 검증
→ 미마운트 → ③ request_cowork_directory 호출 → 성공
→ ④ 보고: ✅ 볼트 마운트 완료
```

**케이스 6: PATH_TRIGGER `~/` 별칭 hit**
```
사용자: "~/ObsidianVault 안에 있는 거 정리"
hit: PATH_TRIGGER `~/` (Rule 0)
→ 즉시 ② 검증
→ 정상 → ④ 보고: ✅ 볼트 이미 마운트됨
```
